...
var SendMail = function(payload, emailType, callback) {
  ...
  if (emailType === "piccniccReciept") {
    ...
    var data = {
      ...
      to: 'jonny.goldstone@googlemail.com', //this needs to be Jonny's email
    ...
    var data = {
      ...
      to: 'jonny.goldstone@googlemail.com',//recieptDetails.email,
    ...
    var data = {
      from: 'Piccnicc Payment <postmaster@' + domain + '>',
      to: 'jonny.goldstone@googlemail.com', //this needs to be Jonny's email
      subject: recieptDetails.firstName + ' order #' + recieptDetails.orderNumber + ' payment failed',
      html: failedPayment
    };
    mailgun.messages().send(data, function (error, body) {
      callback(error);
