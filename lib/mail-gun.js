...
var SendMail = function(payload, destination, callback) {
  var recieptDetails = payload
  if (destination === "piccnicc") {
    var piccniccReciept = formatPiccniccReciept(recieptDetails)
    var data = {
      from: 'Excited User <postmaster@' + domain + '>',
      to: 'ruth@foundersandcoders.com',
      subject: recieptDetails.firstName + ', #' + recieptDetails.orderNumber,
